...
void gatherBaseTypeNames(crow::json::rvalue &packet, std::string &prefix, std::vector< pair<std::string,std::string>> &names);

void gatherBaseTypeNames_Member(crow::json::rvalue &member,  std::string prefix, std::vector< pair<std::string,std::string>> &names) {
...
void adsdatasrc::writeSymbolValue(crow::json::rvalue packet) {
  ...
  for (auto symbolName : symbolNames) {
      ...
      if(info.valid == true){
        ...
        parReqPop->length = info.size;
        parReqPop++;
        dataType_member_base* dt = impl->getType( info.type );
        dt->encode(pdata, symbolName.second, info.size);
        pdata += info.size;
      }
  }
